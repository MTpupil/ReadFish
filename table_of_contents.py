...
class TableOfContents:
    ...
    def __init__(self):
        ...
        self.chapter_patterns = [
            ...
            # 第X部分、第X篇等
            rf'^\s*第{number_pattern}部分(?:[：:\s]+.*)?$',
            
            # 纯英文编号章节：Chapter 1、CHAPTER 001
            r'^\s*[Cc][Hh][Aa][Pp][Tt][Ee][Rr]\s*[\d]+\s*$',

            # Chapter X、CHAPTER X等（兼容编号后带标题）
            r'^\s*[Cc][Hh][Aa][Pp][Tt][Ee][Rr]\s*[\d]+(?:[：:\s]+.*)?$',
            
            # 卷X：卷一、卷二等，兼容只有卷编号
            rf'^\s*卷{number_pattern}(?:[：:\s]+.*)?$',
            
            # 书X：书一、书二等，兼容只有书编号
            rf'^\s*书{number_pattern}(?:[：:\s]+.*)?$',
            
            # 篇X：篇一、篇二等，兼容只有篇编号
            rf'^\s*篇{number_pattern}(?:[：:\s]+.*)?$',
