...
- **快捷键支持**: 提供丰富的快捷键，方便快速操作
  - `Ctrl+F`: 打开搜索窗口
  - `Ctrl+I`: 打开设置窗口
  - `Ctrl+``: 临时固定/取消固定显示阅读器（全局生效）
  - `Ctrl+E`: 显示章节信息和阅读进度悬浮窗
  - `Ctrl+Q`: 退出应用程序
  - `ESC`: 关闭阅读器窗口
...
- **增强搜索**: 优化搜索功能，提升搜索效率
  - 搜索结果显示章节信息，方便定位
  - 默认勾选"只搜索当前位置之后"，避免重复显示已读内容
  - 支持长篇章节目录解析，无章节数量限制
  - 优化搜索效率，减少日志输出影响

### 📊 章节信息悬浮窗
- **实时显示**: 按住Ctrl+E显示当前章节名和阅读进度
- **全书进度**: 显示全书阅读进度百分比
- **本章进度**: 显示当前章节阅读进度
- **智能避障**: 悬浮窗自动调整位置，避免边缘重叠
- **半透明背景**: 增强文字可读性，不影响阅读体验
- **实时更新**: 随着阅读进度自动更新信息
